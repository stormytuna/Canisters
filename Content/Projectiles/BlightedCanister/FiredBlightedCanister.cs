	...
	public override void OnExplode() {
		...
		for (int i = 0; i < 5; i++) {
			...
			Projectile.NewProjectile(Projectile.GetSource_FromThis(), position, velocity,
				ModContent.ProjectileType<BlightedBall>(), Projectile.damage / 3, Projectile.knockBack / 3f,
				Projectile.owner, sign);
		}

		DustHelpers.MakeDustExplosion(Projectile.Center, 10f, DustID.CursedTorch, Main.rand.Next(50, 65), 0f, 15f, 100,
			150, 1f, 1.3f, true);
		DustHelpers.MakeDustExplosion(Projectile.Center, 10f, DustID.CursedTorch, Main.rand.Next(12, 20), 0f, 10f, 100,
			150, 1.3f, 1.6f, true);
		DustHelpers.MakeDustExplosion(Projectile.Center, 10f, DustID.CursedTorch, Main.rand.Next(10, 15), 0f, 3f, 100,
			150, 1f, 1.3f);
