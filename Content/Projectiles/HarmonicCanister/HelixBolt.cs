...
public class HelixBolt : ModProjectile
{
	...
	private ref float AI_FrameCount {
	...
	/// </summary>
	private ref float AI_State {
		get => ref Projectile.ai[1];
	}

	private short DustType {
		get => AI_State == 1f ? DustID.PinkTorch : DustID.PurpleTorch;
	}
	...
	public override void AI() {
		...
		if (firstFrame) {
			if (AI_State == 0f) {
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity, Type,
					Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, -1f);
				AI_State = 1f;
			}
